...
static int write_install_script(const BundleOptions *opts)
{
    ...
    fprintf(fp,
            "#!/bin/sh\n"
            "# Generated by packmule " PACKMULE_VERSION
            " -- offline %s installer\n"
            "set -e\n"
    ...
    if (strcmp(opts->registry_name, "pypi") == 0) {
        /*
         * Write a requirements.txt so pip can install everything in one
         * invocation and respects any dependency ordering it needs.
         */
        char req_path[4096];
        snprintf(req_path, sizeof(req_path), "%s/requirements.txt",
                 opts->output_dir);
        FILE *rfp = fopen(req_path, "w");
        if (!rfp) {
    ...
    } else if (strcmp(opts->registry_name, "npm") == 0) {
        fprintf(fp,
                "for f in \"$DIR\"/*.tgz; do\n"
                "  npm install \"$f\"\n"
                "done\n");

    } else if (strcmp(opts->registry_name, "rpm") == 0) {
        /*
         * Prefer dnf for automatic dependency ordering; fall back to rpm -Uvh
         * on systems that only have the low-level tool.
         */
        fprintf(fp,
        ...
        fprintf(fp, "echo 'No install script available for registry: %s'\n",
                opts->registry_name);
    }
...
int bundle_create(const BundleOptions *opts)
{
    ...
    char *archive_path = pm_malloc(dlen + 8);   /* ".tar.gz\0" = 8 bytes */
